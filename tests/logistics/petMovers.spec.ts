...
test.describe('PetMovers', () => {
    ...
    test('create, update, and delete a PetMover', async ({ page, logisticsApp }) => {
        ...
        try {
            ...
            await pm.clickAdd();
            await pm.fillForm({ name: e2ePetMovers.nameCreate, code: cleanup.code, active: true });
            await pm.saveModal();
            ...
            await pm.expectRowContains(cleanup.code);

            await pm.clickEdit(cleanup.code);
            await pm.fillForm({
            ...
            await pm.clickDelete(cleanup.code);
